    ...
    mailer.sendMail(
      ...
      err => {
        if (err)
          return res
            .status(400)
            .send({ error: "Não foi possível enviar email confirmação!" });
      }
    );
...
router.post("/resetar_senha_confirmar_token", async (req, res) => {
...
router.post("/confirmar_conta", async (req, res) => {
  const { token } = req.body;
  try {
...
router.post("/atualizar_email_confirmar_token", async (req, res) => {
  const { token } = req.body;
  try {
...
router.post("/reverter_email_confirmar_token", async (req, res) => {
  const { token } = req.body;
  try {
